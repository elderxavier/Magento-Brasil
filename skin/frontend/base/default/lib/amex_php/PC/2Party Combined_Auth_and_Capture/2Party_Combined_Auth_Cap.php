...
$cardExp         = $_POST["CardExpYear"].$_POST["CardExpMonth"];
...
if (strlen($cardExp) > 0) {
	$conn->addDigitalOrderField("CardExp", $cardExp);
}
...
$txSource         = $_POST["TxSource"];
$txSourceSubType  = $_POST["TxSourceSubType"];

if (strlen($txSource) > 0) {
	$conn->addDigitalOrderField("TxSource", $txSource);
}

if (strlen($txSourceSubType) > 0) {
	$conn->addDigitalOrderField("TxSourceSubType", $txSourceSubType);
}
...
// Add Card Track Data. TxSource must be "CARDPRESENT" if CardTrack data is present.

$cardTrack1 = $_POST["CardTrack1"];
$cardTrack2 = $_POST["CardTrack2"];
    
if (strlen($cardTrack1) > 0) {
	$conn->addDigitalOrderField("CardTrack1", $cardTrack1);
}
    
if (strlen($cardTrack2) > 0) {
	$conn->addDigitalOrderField("CardTrack2", $cardTrack2);
}

// Perform the 2 party transaction request
// **************************************************************


$orderInfo = $_POST["OrderInfo"];
$purchaseAmount = $_POST["Amount"];


$conn->sendMOTODigitalOrder($orderInfo, $merchantID, $purchaseAmount, "en", "");
...
if (($errorMessage == "") && ($qsiResponseCode == "0")) {
	...
	$captConn->debug = array_key_exists("DebugOn", $_POST) ? true : false;

	// Connect to the Payment Client
	$captConn->connect($_POST["HostName"],$_POST["Port"]);
...
<body>
    ...
    <table align='center' border='0' width='80%'>
        ...
        <tr class='title'>
            <td colspan='2'><p><strong>&nbsp;2-Party Transaction Receipt Fields</strong></p></td>
        </tr>
        <tr class='shade'>
